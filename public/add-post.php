							...
							<div class="card-body flex flex-col gap-6">
								...
								<div class="flex flex-col md:flex-row justify-between gap-4">
									<div class="card w-full">
										<div class="card-body">
											<img src="../images/post.gif" class="w-full">
										</div>
									</div>
									<div class="card w-full">
										<div class="card-body">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    tagSelector.addEventListener('change', () => {
        ...
        if (!selectedTags.includes(selectedValue) && selectedValue !== '') {
            selectedTags.push({
                value: selectedValue,
                text: selectedText
            });
        ...
        // Reset select input ke placeholder
        tagSelector.value = "";
    });

    // Update UI dengan tag yang dipilih
    function updateSelectedTagsUI() {
        selectedTagsContainer.innerHTML = ""; // Bersihkan container
        selectedTags.forEach(tag => {
            const tagElement = document.createElement('div');
            tagElement.className = 'bg-indigo-100 text-indigo-700 px-3 py-1 rounded-full flex items-center gap-2';
            tagElement.innerHTML = `
                <span>${tag.text}</span>
                <button type="button" class="text-indigo-700 hover:text-red-600" onclick="removeTag('${tag.value}')">×</button>
            `;
            selectedTagsContainer.appendChild(tagElement);
        });
    }

    // Update hidden input dengan semua tag yang dipilih
    function updateHiddenInput() {
        tagsInput.value = selectedTags.map(tag => tag.value).join(",");
    }
